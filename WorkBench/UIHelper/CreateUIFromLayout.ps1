Start-Transcript -OutputDirectory "s:\Logs\"
Write-Host "OoOo CREATE UI FROM TEMPLATE OoOo"
...
$FileBrowser = New-Object System.Windows.Forms.OpenFileDialog -Property @{ InitialDirectory = $layoutfolder }
...
#Copy UITemplate folder
Write-Host "- Copying UITemplate folder to new folder $Name ...."
...
# Rename internal reference to UI_TEMPLATE_ID
...
{
	(Get-Content $file.PSPath) |
	Foreach-Object { $_ -replace "UITemplateMenu", "UI$NameMenu"  } |
	Set-Content $file.PSPath	
}
